...
@bot.command(name="engrep", description="Processes .ship file and returns .stl")
async def engrep(ctx):
    if not is_whitelisted(ctx.author.id, ctx.guild.id):
        await ctx.send("womp womp", ephemeral=True)
        log_command('not whitelisted', ctx.author)
        return

    if not ctx.data['data']['options'][0]['value'].lower().endswith('.ship'):
        await ctx.send('... Sir you wanted me to analyze something?', ephemeral=True)
        log_command('engrep - no .ship file', ctx.author)
        return

    attachment = ctx.data['data']['resolved']['attachments'][ctx.data['data']['options'][0]['value']]
    file_path = os.path.join(TEMP_DIR, attachment['filename'])
    await download_attachment(attachment['url'], file_path)

    if file_path.endswith('.fleet'):
        await ctx.send('Sorry sir thats above my pay grade', ephemeral=True)
        log_command('engrep - .fleet file', ctx.author)
        os.remove(file_path)
        return
    elif file_path.endswith('.missile'):
        await ctx.send('Sorry sir thats not my area of expertise', ephemeral=True)
    ...
    if hull_type not in valid_hulls:
        await ctx.send('Sorry sir I havent worked with that hull before', ephemeral=True)
...
@bot.command(name="resize", description="Resizes .png file")
async def resize(ctx):
    ...
    if not ctx.data['data']['options'][0]['value'].lower().endswith('.png'):
        await ctx.send('... Sir you wanted me to resize something?', ephemeral=True)
...
@bot.command(name="log", description="Sends me (sirpoopit) the command log (plz dont use this)")
async def log(ctx):
    if not is_whitelisted(ctx.author.id, ctx.guild.id):
        await ctx.send("womp womp", ephemeral=True)
        log_command('not whitelisted', ctx.author)
    ...
    try:
        user = await bot.http.get_user(SPECIFIC_USER_ID)
        with open(LOG_FILE_PATH, 'rb') as log_file:
            await bot.http.send_message(user.id, file=discord.File(log_file, 'command_log.txt'))
        log_command('log', ctx.author)
    except Exception as e:
        await ctx.send(f"Failed to send log file: {str(e)}", ephemeral=True)

# Event to signify the bot is ready
@bot.command(name="on_ready", description="Bot is ready")
